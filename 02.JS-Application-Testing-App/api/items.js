...
async function getAll(){
    return await fetch(url.getItemsUrl() + '?sortBy=_createdOn%20desc&distinct=recipeName')
    .then(res => res.json())
    .then(items => items)
}
...
async function searchRecipes(query){
    return await fetch(url.getItemsUrl() + `?where=recipeName%20LIKE%20%22${query}%22`)
    .then(res => res.json())
    .then(items => items);
}
